...
def limpiar(valor):
    """Convert NaN/None to empty string and strip text."""
    if pd.isna(valor):
        return ""
...
def draw_wrapped_text(
    ...
    Blank strings create paragraph spacing.
    """
    if text is None:
        return y
...
def draw_document_line(c, item, x, y):
    ...
    Draws either:
    - plain string
    - dict with text/bold/name fields
    """
...
def generar_pdf(titulo, lineas, fecha_emision=""):
    ...
    # Title - automatic, because document types have different titles
